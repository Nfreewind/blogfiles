...
//dev_id是request_irq中的参数
static irqreturn_t my_irq(int irq, void *dev_id) {
	//printk(KERN_EMERG"my_irq>>>>%d\n", irq);
	_irq_count++;
	sprintf(message,"irq_count=%d", _irq_count);
	ev_press = 1;
...
int _devfops_open(struct inode *inode, struct file *file) {
	int ret = 0;
	//请求中断
	if ((ret = request_irq(16, my_irq, IRQF_SHARED, DEVICE_NAME, (void *) message) )!= 0) {
		printk(KERN_EMERG"request_irq err>>>>%d\n", ret);
	}
	return ret;
}
